...
for (let w = 0; w < WORLD.length; w++) {
    ...
    file.innerHTML = `<tbody><tr><th class="world-left"><div class="world-title"><p>${WORLD[w].name}</p><p>${WORLD[w].author}</p></div><p class="world-desc">${WORLD[w].description}</p><p class="world-version">${WORLD[w].version}</p><div class="world-download"><a href="./!db/worlds/${WORLD[w].file}.zip" download>[Download]</a><p>(${WORLD[w].size})</p></div></th><th class="world-right" style="background-image: url('./!db/worlds/!thumbnail/${WORLD[w].thumbnail}')"></th></tr></tbody>`;
